...
export function useVapi(book: IBook) {
    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            if (!ASSISTANT_ID) {
                setLimitError('VAPI assistant is not configured. Please set NEXT_PUBLIC_ASSISTANT_ID environment variable.');
                setStatus('idle');
                return;
            }
            ...
            if (sessionIdRef.current) {
                endVoiceSession(sessionIdRef.current, 0).catch((endErr) => console.error("Failed to rollback session after start failure:", endErr));
            }
            setStatus('idle');
